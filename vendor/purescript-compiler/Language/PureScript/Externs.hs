...
import Paths_purescript_format as Paths
...
data ExternsDeclaration
  -- | A type declaration
  = EDType { edTypeName :: ProperName 'TypeName
           , edTypeKind :: Kind
           , edTypeDeclarationKind :: TypeKind }
  -- | A type synonym
  | EDTypeSynonym { edTypeSynonymName :: ProperName 'TypeName
                  , edTypeSynonymArguments :: [(Text, Maybe Kind)]
                  , edTypeSynonymType :: Type }
  -- | A data construtor
  | EDDataConstructor { edDataCtorName :: ProperName 'ConstructorName
                      , edDataCtorOrigin :: DataDeclType
                      , edDataCtorTypeCtor :: ProperName 'TypeName
                      , edDataCtorType :: Type
                      , edDataCtorFields :: [Ident] }
  -- | A value declaration
  | EDValue { edValueName :: Ident
            , edValueType :: Type }
  -- | A type class declaration
  | EDClass { edClassName :: ProperName 'ClassName
  ...
  | EDInstance { edInstanceClassName :: Qualified (ProperName 'ClassName)
               , edInstanceName :: Ident
               , edInstanceTypes :: [Type]
               , edInstanceConstraints :: Maybe [Constraint] }
  -- | A kind declaration
  | EDKind { edKindName :: ProperName 'KindName }
  deriving (Show)
...
applyExternsFileToEnvironment ExternsFile {..} =
  flip (foldl' applyDecl) efDeclarations
  where
    ...
    applyDecl env (EDDataConstructor pn dTy tNm ty nms) =
      env
      { dataConstructors =
          M.insert (qual pn) (dTy, tNm, ty, nms) (dataConstructors env)
      }
    applyDecl env (EDValue ident ty) =
      env
      { names =
          M.insert
            (Qualified (Just efModuleName) ident)
            (ty, External, Defined)
            (names env)
      }
    applyDecl env (EDClass pn args members cs deps) =
      ...
      { typeClasses =
          M.insert
            (qual pn)
            (makeTypeClassData args members cs deps)
            (typeClasses env)
      }
    ...
    applyDecl env (EDInstance className ident tys cs) =
      ...
      { typeClassDictionaries =
          updateMap
            (updateMap (M.insert (qual ident) dict) className)
            (Just efModuleName)
...
moduleToExternsFile (Module _ _ _ _ Nothing) _ =
  internalError "moduleToExternsFile: module exports were not elaborated"
moduleToExternsFile (Module ss _ mn ds (Just exps)) env = ExternsFile {..}
  where
    ...
    fixityDecl (ValueFixityDeclaration _ (Fixity assoc prec) name op) =
      fmap
        (const (ExternsFixity assoc prec op name))
        (find (findOp getValueOpRef op) exps)
    ...
    typeFixityDecl (TypeFixityDeclaration _ (Fixity assoc prec) name op) =
      fmap
        (const (ExternsTypeFixity assoc prec op name))
        (find (findOp getTypeOpRef op) exps)
    typeFixityDecl _ = Nothing
    findOp ::
         (DeclarationRef -> Maybe (OpName a))
      -> OpName a
      -> DeclarationRef
      -> Bool
    ...
    toExternsDeclaration (TypeRef _ pn dctors) =
      case Qualified (Just mn) pn `M.lookup` types env of
        Nothing ->
          internalError "toExternsDeclaration: no kind in toExternsDeclaration"
        Just (kind, TypeSynonym)
          | Just (args, synTy) <-
             Qualified (Just mn) pn `M.lookup` typeSynonyms env ->
            [EDType pn kind TypeSynonym, EDTypeSynonym pn args synTy]
        ...
        Just (kind, tk@(DataType _ tys)) ->
          ...
          , (dty, _, ty, args) <-
              maybeToList
                (Qualified (Just mn) dctor `M.lookup` dataConstructors env)
          ]
    ...
    toExternsDeclaration (TypeClassRef _ className)
      | Just TypeClassData {..} <-
         Qualified (Just mn) className `M.lookup` typeClasses env
      , Just (kind, TypeSynonym) <-
         Qualified (Just mn) (coerceProperName className) `M.lookup` types env
      , Just (_, synTy) <-
         Qualified (Just mn) (coerceProperName className) `M.lookup`
         typeSynonyms env =
    ...
    toExternsDeclaration (TypeInstanceRef _ ident) =
      ...
      , TypeClassDictionaryInScope {..} <-
          maybeToList (M.lookup (Qualified (Just mn) ident) m2)
      ]
    toExternsDeclaration (KindRef _ pn)
      | Qualified (Just mn) pn `S.member` kinds env = [EDKind pn]
