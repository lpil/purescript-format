...
import System.FilePath
       (makeRelative, takeDirectory, takeDirectory, takeFileName)
...
data ErrorMessage
  ...
  | UnsupportedExport
  | ErrorInModule ModuleIdentifier
                  ErrorMessage
...
guessModuleIdentifier ::
     MonadError ErrorMessage m => FilePath -> m ModuleIdentifier
guessModuleIdentifier filename =
  ModuleIdentifier (takeFileName (takeDirectory filename)) <$>
  guessModuleType (takeFileName filename)
  where
...
data ModuleElement
  = Require JSStatement
            String
            (Either String ModuleIdentifier)
  | Member JSStatement
           Bool
           String
           JSExpression
...
printErrorMessage (UnableToParseModule err) =
  ["The module could not be parsed:", err]
...
withDeps (Module modulePath fn es) = Module modulePath fn (map expandDeps es)
  ...
  -- | Calculate dependencies and add them to the current element.
    ...
    expandDeps (Member n f nm decl _) =
      Member n f nm decl (ordNub $ dependencies modulePath decl)
    expandDeps (ExportsList exps) = ExportsList (map expand exps)
      where
        expand (ty, nm, n1, _) =
          (ty, nm, n1, ordNub (dependencies modulePath n1))
    expandDeps other = other
    dependencies ::
         ModuleIdentifier -> JSExpression -> [(ModuleIdentifier, String)]
...
-- Other constructor.
toModule ::
     forall m. (MonadError ErrorMessage m)
  => S.Set String
  -> ModuleIdentifier
  -> Maybe FilePath
  -> JSAST
...
toModule mids mid filename top
  | JSAstProgram smts _ <- top =
    Module mid filename <$> traverse toModuleElement smts
...
getExportedIdentifiers ::
     (MonadError ErrorMessage m) => String -> JSAST -> m [String]
getExportedIdentifiers mname top
  ...
  where
    ...
    go stmt
      ...
      | Just (True, name, _) <- matchMember stmt = pure [name]
      | otherwise = pure []
    toIdent (JSPropertyNameandValue name _ [_]) = extractLabel' name
    toIdent _ = err UnsupportedExport
    extractLabel' = maybe (err UnsupportedExport) pure . extractLabel
...
matchExportsAssignment stmt
  ...
  , JSIdentifier _ "exports" <- exports
  , JSObjectLiteral _ props _ <- decl = Just props
  | otherwise = Nothing
...
compile modules entryPoints = filteredModules
  ...
  -- | The vertex set
    ...
    --    inlined wherever they are used inside other module elements.
      where
        toVertices ::
             ModuleIdentifier -> ModuleElement -> [(ModuleElement, Key, [Key])]
        ...
        toVertices p m@(ExportsList exps) = mapMaybe toVertex exps
          where
            ...
            toVertex (RegularExport nm, nm1, _, ks)
              | nm /= nm1 = Just (m, (p, nm1), ks)
            toVertex _ = Nothing
        toVertices _ _ = []
  ...
  -- | The set of vertices reachable from an entry point
    ...
    filteredModules = map filterUsed modules
      where
        ...
        filterUsed (Module mid fn ds) =
          Module mid fn (map filterExports (go ds))
          where
      ...
      -- | Filter out the exports for members which aren't used.
            ...
            filterExports (ExportsList exps) =
              ExportsList (filter (\(_, nm, _, _) -> isKeyUsed (mid, nm)) exps)
            filterExports me = me
...
codeGen optionsMainModule optionsNamespace ms outFileOpt =
  (fmap sourceMapping outFileOpt, rendered)
  where
    rendered =
      renderToString
        (JSAstProgram
           (prelude :
            concatMap fst modulesJS ++ maybe [] runMain optionsMainModule)
           JSNoAnnot)
    ...
    sourceMapping outFile =
      ...
      where
        ...
        offsets (m, n) (Right d:rest) =
          map ((m +) &&& (n +)) [0 .. d - 1] ++ offsets (m + d, n + d) rest
        offsets _ _ = []
    ...
    moduleToJS (Module mn _ ds) =
      (wrap (moduleName mn) (indent (concat jsDecls)), lengths)
      where
            ...
            [ JSVariable
                ...
                (cList
                   [ JSVarInitExpression
                       (JSIdentifier sp nm)
                       (JSVarInit sp $
                        either require (moduleReference sp . moduleName) req)
                   ])
                (JSSemi JSNoAnnot)
            ]
  ...
  -- comma lists are reverse-consed
    ...
    prelude =
      JSVariable
        (JSAnnot
           tokenPosnEmpty
           [ CommentA tokenPosnEmpty $
             "// Generated by purs bundle " ++ showVersion Paths.version
      ...
      where
        ...
        addAnn a (JSExpressionStatement (JSStringLiteral ann s) _) =
          JSExpressionStatement
            (JSStringLiteral (appendAnn a ann) s)
            (JSSemi JSNoAnnot)
        addAnn _ x = x
        appendAnn a JSNoAnnot = JSAnnot tokenPosnEmpty [a]
        appendAnn a (JSAnnot _ anns) =
          JSAnnot tokenPosnEmpty (a : anns ++ [WhiteSpace tokenPosnEmpty "  "])
...
bundleSM inputStrs entryPoints mainModule namespace outFilename = do
  ...
  forM_ mainModule $ \mname ->
    when
      (mname `notElem` map (moduleName . mid) inputStrs)
      (throwError (MissingMainModule mname))
  forM_ entryPoints $ \mIdent ->
    when
      (mIdent `notElem` map mid inputStrs)
      (throwError (MissingEntryPoint (moduleName mIdent)))
  input <-
    forM inputStrs $ \(ident, filename, js) -> do
      ast <-
        either (throwError . ErrorInModule ident . UnableToParseModule) pure $
        parse js (moduleName ident)
      return (ident, filename, ast)
  let mids = S.fromList (map (moduleName . mid) input)
  modules <-
    traverse (fmap withDeps . (\(a, fn, c) -> toModule mids a fn c)) input
...
bundle ::
  ...
  -> String -- ^ The namespace (e.g. PS).
  -> m String
bundle inputStrs entryPoints mainModule namespace =
  snd <$>
  bundleSM
    (map (\(a, b) -> (a, Nothing, b)) inputStrs)
    entryPoints
    mainModule
    namespace
    Nothing
