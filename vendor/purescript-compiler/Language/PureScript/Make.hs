...
import qualified Paths_purescript_format as Paths
import SourceMap
...
import System.FilePath
       ((</>), makeRelative, normalise, replaceExtension, splitPath,
        takeDirectory)
...
rebuildModule MakeActions {..} externs m@(Module _ _ moduleName _ _) = do
  ...
  lint withPrim
  ((Module ss coms _ elaborated exps, env'), nextVar) <-
    runSupplyT 0 $ do
  ...
  (deguarded, nextVar') <- runSupplyT nextVar $ do desugarCaseGuards elaborated
  regrouped <-
    createBindingGroups moduleName . collapseBindingGroups $ deguarded
...
make ma@MakeActions {..} ms = do
  ...
  barriers <-
    zip (map getModuleName sorted) <$>
    replicateM (length ms) ((,) <$> C.newEmptyMVar <*> C.newEmptyMVar)
  for_ sorted $ \m ->
    fork $ do
      let deps =
            fromMaybe
              (internalError "make: module not found in dependency graph.")
              (lookup (getModuleName m) graph)
      buildModule
        barriers
        (importPrim m)
        (deps `inOrderOf` map getModuleName sorted)
  ...
  (_, externs) <-
    unzip .
    fromMaybe
      (internalError "make: externs were missing but no errors reported.") .
    sequence <$>
    for barriers (takeMVar . fst . snd)
  return externs
  where
  ...
  -- Sort a list so its elements appear in the same order as in another list.
    ...
    inOrderOf xs ys =
      let s = S.fromList xs
      in filter (`S.member` s) ys
    buildModule ::
         [( ModuleName
          , ( C.MVar (Maybe (MultipleErrors, ExternsFile))
            , C.MVar (Maybe MultipleErrors)))]
      -> Module
      -> [ModuleName]
      -> m ()
    buildModule barriers m@(Module _ _ moduleName _ _) deps =
      flip catchError (markComplete Nothing . Just) $
    ...
    -- MVars for the module's dependencies.
       do
        mexterns <-
          ...
          traverse
            (readMVar .
             fst .
             fromMaybe (internalError "make: no barrier") . flip lookup barriers)
            deps
        case mexterns of
          Just (_, externs) -> do
            ...
            dependencyTimestamp <-
              maximumMaybe <$>
              traverse (fmap shouldExist . getOutputTimestamp) deps
            inputTimestamp <- getInputTimestamp moduleName
            let shouldRebuild =
                  case (inputTimestamp, dependencyTimestamp, outputTimestamp) of
      ...
      where
        markComplete ::
             Maybe (MultipleErrors, ExternsFile) -> Maybe MultipleErrors -> m ()
        markComplete externs errors = do
          putMVar
            (fst $
             fromMaybe (internalError "make: no barrier") $
             lookup moduleName barriers)
            externs
          putMVar
            (snd $
             fromMaybe (internalError "make: no barrier") $
  ...
  -- Make sure a dependency exists
    ...
    shouldExist _ =
      internalError "make: dependency should already have been built."
...
readTextFile path =
  makeIO (const (ErrorMessage [] $ CannotReadFile path)) $ B.readFile path
...
buildMakeActions outputDir filePathMap foreigns usePrefix =
  ...
  where
    getInputTimestamp ::
         ModuleName -> Make (Either RebuildPolicy (Maybe UTCTime))
    getInputTimestamp mn = do
      let path =
            fromMaybe (internalError "Module has no filename in 'make'") $
            M.lookup mn filePathMap
    ...
    getOutputTimestamp mn = do
      ...
      min3 <$> getTimestamp jsFile <*> getTimestamp externsFile <*>
        getTimestamp coreFnFile
            ...
            | otherwise -> do
              ...
              return $
                ...
                Imp.App
                  ...
                  [Imp.StringLiteral Nothing "./foreign"]
          Nothing
            | requiresForeign m ->
              throwError . errorMessage $ MissingFFIModule mn
            | otherwise -> return Nothing
      rawJs <- J.moduleToJs m foreignInclude
      dir <-
        lift $
        makeIO
          (const (ErrorMessage [] $ CannotGetFileInfo "."))
          getCurrentDirectory
      sourceMaps <- lift $ asks optionsSourceMaps
      let (pjs, mappings) =
            if sourceMaps
              then prettyPrintJSWithSourceMaps rawJs
              else (prettyPrintJS rawJs, [])
      let filePath = T.unpack (runModuleName mn)
          jsFile = outputDir </> filePath </> "index.js"
          ...
          prefix =
            [ "Generated by purs version " <> T.pack (showVersion Paths.version)
            | usePrefix
            ]
          js = T.unlines $ map ("// " <>) prefix ++ [pjs]
          mapRef =
            if sourceMaps
              then "//# sourceMappingURL=index.js.map\n"
              else ""
      lift $ do
        writeTextFile jsFile (B.fromStrict $ TE.encodeUtf8 $ js <> mapRef)
        for_
          (mn `M.lookup` foreigns)
          (readTextFile >=> writeTextFile foreignFile)
    ...
    genSourceMap dir mapFile extraLines mappings = do
      let pathToDir =
            iterate (".." </>) ".." !! length (splitPath $ normalise outputDir)
          sourceFile =
            case mappings of
              (SMap file _ _:_) ->
                Just $ pathToDir </> makeRelative dir (T.unpack file)
              _ -> Nothing
      let rawMapping =
            SourceMapping
            { smFile = "index.js"
    ...
    getTimestamp path =
      makeIO (const (ErrorMessage [] $ CannotGetFileInfo path)) $ do
        exists <- doesFileExist path
        traverse (const $ getModificationTime path) $ guard exists
    writeTextFile :: FilePath -> B.ByteString -> Make ()
    writeTextFile path text =
      makeIO (const (ErrorMessage [] $ CannotWriteFile path)) $ do
        mkdirp path
...
checkForeignDecls m path = do
  ...
  js <-
    either (errorParsingModule . Bundle.UnableToParseModule) pure $
    JS.parse (BU8.toString (B.toStrict jsStr)) path
  ...
  unless (null unusedFFI) $
    tell . errorMessage . UnusedFFIImplementations mname $ S.toList unusedFFI
  ...
  where
    ...
    errorParsingModule =
      throwError . errorMessage . ErrorParsingFFIModule path . Just
    ...
    errorInvalidForeignIdentifiers =
      throwError .
      mconcat . map (errorMessage . InvalidFFIIdentifier mname . T.pack)
  ...
  -- identifier should be enough.
    ...
    parseIdent str = try (T.pack str)
      where
        try s =
          either (const (Left str)) Right $ do
            ts <- PSParser.lex "" s
